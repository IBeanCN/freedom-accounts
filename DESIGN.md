...
### 11.8 账号行操作折叠（2026-09-24）

账号操作列固定为「执行 / 日志 / 更多」三个动作位；换指纹、指纹检测、刷新 Token、编辑和删除收入 `el-dropdown`。低频删除在菜单内保留危险色。列宽保持紧凑，禁止因运行态出现而把额外按钮挤进行内。

登录任务或指纹检测进行中时，第一动作位的「执行」被 24px 圆形 `quiet-stop` 图标钮替换，不再保留禁用「执行」加独立「停止」的双入口。悬停显示「停止任务」或「停止指纹检测」；默认只给 warning 语义色和透明底，悬停后才出现 soft 底色。若登录任务和检测并存，登录停止占用第一动作位，检测停止继续保留在「更多」菜单内。
